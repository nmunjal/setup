...
if [ $system_type -eq $redhat_type ]; then
    ...
    sudo yum -y install spawn-fcgi fcgi fcgi-devel gcc-c++ libmemcached-devel openssl098e
elif [ $system_type -eq $debian_type ]; then
    sudo apt-get -y install libfcgi-dev spawn-fcgi g++ libmemcached-dev libssl0.9.8
fi
let "steps++"
